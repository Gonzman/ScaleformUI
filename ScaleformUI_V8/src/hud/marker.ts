...
export class Marker {
    ...
    public draw() {
        ...
        if (
            this.isInDrawRange(ped, coords) &&
            this.config.placeOnGround &&
            this.config.position.z !== this.config.height + 0.1
        ) {
        ...
        DrawMarker(
            this.config.type ?? MarkerType.UpsideDownCone,
        ...
        if (this.config.checkZ) {
            this.updateIsInMarkerState(
                coords.isInsideSphere(this.config.position, this.config.scale || Vector3.create(1))
            );
        } else {
    ...
    public isInDrawRange(_optPed?: number, _optCoords?: Vector3) {
        ...
        return (
            (this.config.checkZ ? coords.distance(this.config.position) : coords.distanceNoZ(this.config.position)) <=
            (this.config.distance ?? 250)
        );
    }
    ...
    private getHandlersForMarkerState(state: boolean): [MarkerEventHandlerSig[], "IN" | "OUT"] {
        ...
        return [
            this.eventHandlers.filter(([onAction]) => [cAction, "BOTH"].includes(onAction)).map((x) => x[1]),
            cAction
        ];
    }
